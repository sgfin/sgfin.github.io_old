...
- Third, a false dichotomy between ML and stats minimizes the wide -- and critical to modeling decisions -- variation _within_ each purported class. It's also silly. Consider two examples:
  - First, consider the following Schroedinger's "am I doing stats research or ML research" experiment, narrated by my dichotomy-lovin'-strawman: Implement a 1-layer, fully connected neural net in Pytorch. Now, add a random variable that, when equal to one, toggles on a second layer -- (Narrator: you now have a quantum state of stats research and _profoundly different_ ML research!)  Now instead, add a thousand super crazy complicated interaction terms and an elastic net penalty -- (Narrator: meh, a trivial difference!) Now, form a research team to integrate it into an autonomous car -- (Narrator: meh, still the same vanilla stats research.) Note:  Before you rag me too hard for this one, be sure to read the next section.
